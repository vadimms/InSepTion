...
data_path <- "./data/UPMC_hd__full_fluidbins.csv"
data <- read.csv(data_path)

# relabel ids
data$hosp_id <- as.factor(as.numeric(as_factor(data$hosp_id)))
data$id <- as.factor(as.numeric(as_factor(data$id)))
data$empi_id <- as.factor(as.numeric(as_factor(data$empi_id)))

data_subset <- head(data, 1000)
...
tmle_fit <- tmle3(tmle_spec, data_subset, node_list, learner_list)
